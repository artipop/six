...
@Serializable
data class TilingWorkspace(
    @Serializable(with = UuidSerializer::class)
    val id: UUID = UUID.randomUUID(),
    /** Optional. A named one survives running out of windows. */
    val name: String = "",
    val columns: List<TilingColumn> = emptyList(),
    ...
    val focusedColumn: TilingColumn? get() = columns.getOrNull(focus)
}
...
@Serializable
data class TilingStrip(
    val workspaces: List<TilingWorkspace> = listOf(TilingWorkspace()),
