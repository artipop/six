...
/**
 * Scrollable tiling: columns laid out along a workspace, workspaces stacked across it.
 ...
 *
 * The Mac's `TilingLayout` is an `@Observable` class that mutates in place. Here it is an immutable
 * data class and every operation returns a new one, because that is what Compose wants: one
 * `StateFlow<TilingLayout>` in a ViewModel, recomposition driven by equality, and transitions that are
 ...
 * itself out differently on two devices is a strip that lost its place. `TilingLayoutGeometryTest` is
...
data class TilingLayout(
    ...
    val strips: Map<UUID, TilingStrip> = emptyMap(),
) {
    ...
    val strip: TilingStrip get() = strips[activeProfileId] ?: TilingStrip()
    val workspaces: List<TilingWorkspace> get() = strip.workspaces
    val focusedWorkspaceIndex: Int
        get() = strip.focus.coerceIn(0, max(0, strip.workspaces.size - 1))
    val focusedWorkspace: TilingWorkspace? get() = strip.workspaces.getOrNull(strip.focus)
    ...
    fun stripFor(profileId: UUID): TilingStrip = strips[profileId] ?: TilingStrip()
     ...
     */
    val showsFill: TilingFill get() = if (isOverview) TilingFill.TILED else fill

    /** One column, one screen: no gaps and no title bars, in both filling modes. */
    val fillsViewport: Boolean get() = showsFill != TilingFill.TILED

    /** Only fullscreen takes the top bar with it. */
    val showsFullscreen: Boolean get() = showsFill == TilingFill.SCREEN
    ...
    fun columnFrames(workspace: TilingWorkspace): List<Rect> {
    ...
    fun contentWidth(workspace: TilingWorkspace): Double {
     ...
     */
    private fun offsetBounds(workspace: TilingWorkspace): ClosedFloatingPointRange<Double> {
    ...
    private fun clampOffset(offset: Double, workspace: TilingWorkspace): Double {
    ...
    fun resolvedOffset(workspace: TilingWorkspace): Double =
    ...
    private fun scrollFocusIntoView(workspace: TilingWorkspace): TilingWorkspace {
        val frames = columnFrames(workspace)
    ...
    private fun mutate(profileId: UUID = activeProfileId, body: (TilingStrip) -> TilingStrip): TilingLayout {
        val next = normalize(body(strips[profileId] ?: TilingStrip()))
        return copy(strips = strips + (profileId to next))
    }

    private fun mutateWorkspace(index: Int, body: (TilingWorkspace) -> TilingWorkspace): TilingLayout =
        mutate { s ->
            val ws = s.workspaces.getOrNull(index) ?: return@mutate s
            s.copy(workspaces = s.workspaces.replacing(index, body(ws)))
        }

    private fun mutateFocusedWorkspace(body: (TilingWorkspace) -> TilingWorkspace): TilingLayout =
        mutateWorkspace(strip.focus, body)

    /**
     * Keeps exactly one trailing empty workspace and drops the empty ones in between — dynamic
     * workspaces. A named workspace stays even when it is empty.
     */
    private fun normalize(s: TilingStrip): TilingStrip {
        ...
        return TilingStrip(workspaces = kept, focus = focus)
    }

    /** Puts every workspace of one strip back under its focused window. */
    private fun recenterStrip(profileId: UUID): TilingLayout =
        mutate(profileId) { s ->
     ...
     */
    fun recenterStrips(): TilingLayout {
        var result = this
     ...
     */
    fun restore(saved: Map<UUID, TilingStrip>): TilingLayout {
    ...
    fun setActiveProfile(profileId: UUID): TilingLayout =
        if (profileId == activeProfileId) this
        else copy(activeProfileId = profileId).recenterStrip(profileId)

    fun setCentersFocus(value: Boolean): TilingLayout =
    ...
    fun insertColumn(
        ...
        workspace: Int? = null,
        focus: Boolean = true,
    ): TilingLayout = mutate(profileId) { s ->
        ...
        var next = ws.copy(
            columns = ws.columns.inserting(at, TilingColumn(tabId)),
        )
    ...
    fun removeColumn(tabId: UUID): TilingLayout = mutate { s ->
    ...
    fun focus(tabId: UUID): TilingLayout = mutate { s ->
        val w = s.workspaces.indexOfFirst { ws -> ws.columns.any { it.tabId == tabId } }
        if (w < 0) return@mutate s
        val ws = s.workspaces[w]
        val index = ws.columns.indexOfFirst { it.tabId == tabId }
    ...
    fun focusColumn(delta: Int): TilingLayout = mutateFocusedWorkspace { ws ->
        if (ws.columns.isEmpty()) ws
        else scrollFocusIntoView(ws.copy(focus = (ws.focus + delta).coerceIn(0, ws.columns.size - 1)))
    }

    fun focusColumnEdge(last: Boolean): TilingLayout = mutateFocusedWorkspace { ws ->
        if (ws.columns.isEmpty()) ws
    ...
    fun moveColumn(delta: Int): TilingLayout = mutateFocusedWorkspace { ws ->
    ...
    fun moveColumn(tabId: UUID, profileId: UUID, toWorkspace: Int): TilingLayout =
        mutate(profileId) { s ->
            ...
            while (destinationIndex >= workspaces.size) workspaces.add(TilingWorkspace())
     ...
     */
    fun panStrip(delta: Double): TilingLayout {
    ...
    fun snapFocusToView(): TilingLayout = mutateFocusedWorkspace { ws ->
    ...
    fun focusWorkspace(delta: Int): TilingLayout =
        mutate { s -> s.copy(focus = (s.focus + delta).coerceIn(0, s.workspaces.size - 1)) }

    fun focusWorkspaceAt(index: Int): TilingLayout =
    ...
    fun moveColumnToWorkspace(delta: Int): TilingLayout = mutate { s ->
        ...
        if (target >= workspaces.size) workspaces.add(TilingWorkspace())
    ...
    fun renameWorkspace(index: Int, name: String): TilingLayout =
    ...
    fun workspaceIndexNamed(
        ...
        val next = mutate(profileId) { s ->
            ...
            val workspaces = if (last != null && last.isEmpty && last.name.isEmpty()) {
                ...
                s.workspaces + TilingWorkspace(name = wanted)
            }
