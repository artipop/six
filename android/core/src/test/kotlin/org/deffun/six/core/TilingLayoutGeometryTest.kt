 ...
 *
 * A port of `Tests/SixCoreTests/TilingLayoutGeometryTests.swift`, test for test. Like the original it
 * is written against the *intent* stated in `TilingLayout`'s own comments rather than against the
 ...
 */
class TilingLayoutGeometryTest {

    private fun layout(viewport: Size = Size(1600.0, 1000.0)): TilingLayout =
        TilingLayout().updateViewport(viewport)

    private fun workspace(count: Int): TilingWorkspace =
        TilingWorkspace(columns = List(count) { TilingColumn(UUID.randomUUID()) })
    ...
    fun gapIsAFractionOfTheViewport() {
        ...
        assertEquals(Math.round(1600 * TilingLayout.GAP_FRACTION).toDouble(), small.gap)
    ...
    fun gapHasAFloorForTinyViewports() {
        assertEquals(TilingLayout.MINIMUM_GAP, layout(Size(300.0, 300.0)).gap)
    }
    ...
    fun emptyWorkspaceHasNoContent() {
        val layout = layout()
        assertTrue(layout.columnFrames(TilingWorkspace()).isEmpty())
        assertEquals(0.0, layout.contentWidth(TilingWorkspace()))
    }
    ...
    fun fillingTakesTheWholeViewportAndIsReversible() {
        for (fill in listOf(TilingFill.WINDOW, TilingFill.SCREEN)) {
            val layout = layout()
            ...
            val restored = filled.setFill(TilingFill.TILED)
    ...
    fun overviewShowsTiledGeometry() {
        val layout = layout().setFill(TilingFill.SCREEN).setOverview(true)

        assertEquals(TilingFill.TILED, layout.showsFill)
