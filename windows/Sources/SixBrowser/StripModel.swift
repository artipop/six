...
/// The row's own state: `TilingLayout` plus what a column needs to draw itself and, once it is
/// live, to load.
///
...
/// writes, under the same key. The row and its mechanics — open, close, focus, move, the
/// workspaces stacked above and below it — are the same shape here as on every other front because
/// `TilingLayout` is the same code, unchanged, imported the way the Linux front already does:
...
public final class StripModel {
    ...
    public static let shared = StripModel()
    ...
    /// The profiles, and which one is on screen. `TilingLayout` already keeps a strip per profile, so
    ...
    /// wires up — an embedder in an off-screen page — has nothing to do with a row.
    ...
    /// request arrives in is this front's (`StripWebView.onMediaRequest`).
    ...
    private init() {
        ...
        if !restore() {
            ...
            // A run pointed at a test page lands on it, restored row or not — otherwise `SIX_URL`
    ...
    /// has no string catalog yet, and the row's own "New Tab" is already in the same boat.
    ...
    /// A profile whose row is empty stays empty when it comes up — the Mac's rule
    ...
    /// Every column in every profile — how `StripLiveView` tells a column that was closed from one
    ...
    /// The whole strip at once, every workspace stacked, scaled down — a way of *looking* at the row
    /// rather than a second arrangement of it, which is the Mac's `TilingStripView` reading of the same
    ...
    /// A column on an address of its own — a row of the history, or `SIX_URL` over a restored row.
    ...
    /// Where a column is, or the start page for one that has not reported anywhere yet — `StripWindow`
    ...
    /// reloading the page it was on is exactly that case: the title is not new to the row, only to
    ...
    public func pageDidFinishLoading(_ tabID: UUID, url: String, title: String) {
        ...
        // `didFinishNavigation` is not the moment a title exists — see `StripWebView.title` — so the
    ...
    /// `StripWebView` installs, because the engine this front runs has no MediaStream to call it
