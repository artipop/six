...
/// something to say.
///
/// Split from `StripChrome` because that file is already the whole bar, and because these two draw
/// from a different source — everything else in the bar is the row's state, and this is the page's.
/// The metrics, the palette and the GDI helpers are still `StripChrome`'s.
extension StripWindow {
    ...
    ///
    /// It is a real line of chrome and not an overlay: `topChromeHeight` counts it, so the row
