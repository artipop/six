...
/// `WM_KEYDOWN` into `KeyBindings`, by way of `StripKeyLookup` — the Win32 half of what
/// `six/Input/KeyEvents.swift` does with an `NSEvent` on the Mac. Nothing here knows what a binding
/// *does*; that is `StripModel`'s.
extension StripWindow {
    ...
    func handleKeyDown(virtualKey: Int32, lParam: LPARAM) -> Bool {
        guard let key = Self.stripKey(virtualKey: virtualKey,
                                     scanCode: Self.scanCode(virtualKey: virtualKey, lParam: lParam))
        else { return false }

        var modifiers: StripKeyModifiers = []
        if SixStripKeyDown(Int32(VK_CONTROL)) != 0 { modifiers.insert(.control) }
        if SixStripKeyDown(Int32(VK_MENU)) != 0 { modifiers.insert(.alt) }
        if SixStripKeyDown(Int32(VK_SHIFT)) != 0 { modifiers.insert(.shift) }

        guard let action = StripKeyLookup.action(for: key, modifiers: modifiers,
    ...
    private func perform(_ action: StripKeyAction) {
        switch action {
    ...
    static func scanCode(virtualKey: Int32, lParam: LPARAM) -> Int32 {
        let fromMessage = SixStripScanCode(lParam)
    ...
    private static func stripKey(virtualKey: Int32, scanCode: Int32) -> StripKey? {
        switch virtualKey {
...
///
/// They are matched by **scan code**, like the letter bindings in `stripKey` above and for the same
...
extension StripWindow {
    ...
    func handleChromeKey(virtualKey: Int32, lParam: LPARAM) -> Bool {
        ...
        guard SixStripKeyDown(Int32(VK_CONTROL)) != 0, SixStripKeyDown(Int32(VK_MENU)) == 0 else { return false }
