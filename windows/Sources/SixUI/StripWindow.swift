...
/// One Win32 window: the row, drawn with GDI, and the input that drives it.
///
...
/// bookkeeping is `CStripInterop`'s; what arrives here is already a `StripWindow`.
@MainActor
public final class StripWindow {
    public private(set) var hwnd: HWND?
    let model = StripModel.shared

    /// See `StripInput.handleWheel` for why a notch is accumulated rather than acted on as it lands.
    ...
    /// display. Everything `StripChrome` measures is multiplied by it; the row itself is laid out in
    /// real pixels by `TilingLayout` and needs no such conversion, because its columns are fractions of
    ...
    var webViews: [Foundation.UUID: StripWebView] = [:]
    ...
    /// Pictures of pages, loaded or taken (`StripThumbnails`), and the columns known to have none.
    ...
    var listPanel: StripListPanel?
    /// A page's `alert()`, `confirm()` or `prompt()` on screen, which column it belongs to, and the
    /// ones waiting behind it (`StripPageDialog`).
    var pageDialog: StripPageDialog?
    var pageDialogTab: Foundation.UUID?
    var waitingDialogs: [QueuedPageDialog] = []
    /// The release of a click the row took for itself — a middle or `Ctrl`-click that opened a link
    /// (`openLinkIfAsked`) — so the page is not handed the second half of a click it never saw.
    var swallowedRelease: UINT?
    /// Files pages handed over (`StripDownloads`), and its list while it is open.
    lazy var downloads = StripDownloads { [weak self] in self?.downloadsChanged() }
    weak var downloadsPanel: StripListPanel?
    ...
    lazy var translation = StripTranslation { [weak self] in self?.invalidate() }
    ...
    /// (`StripFrame`), so their hover and their press are ours to remember.
    ...
    static let className = "SixStripWindow"
    ...
    public func create(instance: HINSTANCE) -> Bool {
        ...
        wc.lpfnWndProc = stripWindowProc
        wc.hInstance = instance
        wc.hCursor = SixStripArrowCursor()
        ...
        let created = Self.className.withCString(encodedAs: UTF16.self) { classNamePtr in
            "six".withCString(encodedAs: UTF16.self) { titlePtr in
                ...
                // `WS_CLIPCHILDREN` so the row's own painting stops at the address field and at the
        ...
        // takes its title bar over (`StripFrame`). Without `SWP_FRAMECHANGED` the caption stays until
        ...
        _ = embedding // the model has an embedder from here on; see `StripEmbedding`
    ...
    public func show() {
        ...
        // alongside the row and takes the keyboard: clicks still land (they route by cursor
    ...
    /// The row's keys and its ⌥-scroll, taken out of the queue before the window they were aimed at
    ...
    /// working the moment you use the page is not a shortcut. Before this, every row binding worked
    ...
    func route(_ message: MSG) -> Bool {
        ...
        // Escape are its subclass procedure's business (`AddressBar`), not the row's.
        ...
        case WM_LBUTTONDOWN, WM_RBUTTONDOWN, WM_MBUTTONDOWN:
            if target != hwnd {
                ...
                // A middle or `Ctrl`-click on a link is the row's to answer, not the page's.
        ...
        case WM_KEYDOWN, WM_SYSKEYDOWN:
            ...
            if ProcessInfo.processInfo.environment["SIX_UI_DEBUG"] == "1" {
                ...
                let trace = "[six] key vk=\(message.wParam) scan=\(SixStripScanCode(message.lParam)) " +
                    "ctrl=\(SixStripKeyDown(Int32(VK_CONTROL))) alt=\(SixStripKeyDown(Int32(VK_MENU))) " +
                    "shift=\(SixStripKeyDown(Int32(VK_SHIFT))) on=\(onPage ? "page" : "row") " +
                    "taken=\(handled)\n"
                FileHandle.standardError.write(Data(trace.utf8))
            }
            return handled
        case WM_MOUSEWHEEL:
            guard SixStripKeyDown(Int32(VK_MENU)) != 0 else { return false }
            handleWheel(delta: Int32(SixStripWheelDelta(message.wParam)), horizontal: false)
            return true
        case WM_MOUSEHWHEEL:
            guard SixStripKeyDown(Int32(VK_MENU)) != 0 else { return false }
            handleWheel(delta: Int32(SixStripWheelDelta(message.wParam)), horizontal: true)
            return true
        default:
            return false
        }
    ...
    var focusedWebView: StripWebView? {
    ...
    func handle(message: UINT, wParam: WPARAM, lParam: LPARAM) -> LRESULT? {
        ...
        // MARK: The frame (StripFrame)

        case WM_NCCALCSIZE:
            return handleNCCalcSize(wParam: wParam, lParam: lParam)

        case WM_NCHITTEST:
            return handleNCHitTest(screenX: Int32(SixStripPointX(lParam)), screenY: Int32(SixStripPointY(lParam)))
        ...
        case WM_SIZE:
            let width = Int(SixStripLoWord(lParam))
            let height = Int(SixStripHiWord(lParam))
            // `TilingLayout`'s viewport is the row's canvas alone; the bar above it is
            // `StripChrome`'s business, not the layout's.
            let stripHeight = max(0, height - Int(topChromeHeight))
            _ = model.updateViewport(CGSize(width: width, height: stripHeight))
            layoutAddressBar()
        ...
        // `StripChrome.drawAddressField` draws behind it.
        ...
        case WM_LBUTTONDOWN:
            handleClick(x: Int(SixStripPointX(lParam)), y: Int(SixStripPointY(lParam)))
            return 0
        ...
        case WM_SETCURSOR:
            guard let hwnd, Int32(SixStripLoWord(lParam)) == HTCLIENT else { return nil }
            var point = POINT()
            GetCursorPos(&point)
            ScreenToClient(hwnd, &point)
            guard chromeAction(x: Int(point.x), y: Int(point.y)) != nil else { return nil }
            SetCursor(SixStripHandCursor())
            return 1
        ...
        case WM_MOUSEWHEEL:
            handleWheel(delta: Int32(SixStripWheelDelta(wParam)), horizontal: false)
            return 0

        case WM_MOUSEHWHEEL:
            handleWheel(delta: Int32(SixStripWheelDelta(wParam)), horizontal: true)
            return 0
...
private nonisolated func stripWindowProc(
    _ hwnd: HWND?, _ message: UINT, _ wParam: WPARAM, _ lParam: LPARAM
) -> LRESULT {
    if Int32(message) == WM_NCCREATE {
        if let hwnd, let params = SixStripCreateParams(lParam) {
            SixStripSetUserData(hwnd, params)
        }
