    ...
    func callAsync(_ body: String, input: String) async throws -> String {
        try await withCheckedThrowingContinuation { continuation in
            guard let script = Self.wkString(body), let key = Self.wkString("input"),
                ...
                continuation.resume(throwing: StripScriptError.noPage)
                return
            }
            ...
            WKPageCallAsyncJavaScript(page, script, arguments, nil, reply) { result, error, context in
                ...
                if let error {
                    reply.resume(throwing: StripScriptError.javaScript(
                        StripWebView.string(from: WKErrorCopyLocalizedDescription(error))))
                    return
                }
                ...
                reply.resume(returning: StripWebView.string(from: OpaquePointer(result)))
            }
        }
    }
...
nonisolated enum StripScriptError: LocalizedError {
...
extension StripWebView: PageScriptRunner {
