...
/// of the queue by `route`, the same way the row's are, because an `EDIT` and a `LISTBOX` each have
/// their own idea of what those keys do.
@MainActor
final class StripListPanel {
    struct Row {
    ...
    private func paint() {
        ...
        if searchable {
            let pill = searchPill
            let brush = CreateSolidBrush(StripWindow.addressFieldColor)
            let pen = CreatePen(Int32(PS_SOLID), 1, StripWindow.addressBorderColor)
        ...
        if rows.isEmpty {
            ...
            text(hdc, emptyText, in: RECT(left: list.left + px(32), top: middle - px(24),
                 ...
                 font: uiFont, color: StripWindow.dimLabelColor, format: DT_CENTER | DT_WORDBREAK)
        }
        text(hdc, hint, in: hintRect, font: smallFont, color: StripWindow.dimLabelColor,
    ...
    private func drawItem(_ item: DRAWITEMSTRUCT) {
        ...
        fill(hdc, item.rcItem, selected ? StripWindow.chipColor : StripWindow.backgroundColor)
        SetBkMode(hdc, TRANSPARENT)
        ...
        text(hdc, row.title, in: RECT(left: rect.left + px(14), top: rect.top + px(4), right: rect.right - px(14), bottom: middle + px(1)),
             font: strongFont, color: StripWindow.textColor, format: DT_LEFT | DT_BOTTOM | DT_SINGLELINE | DT_END_ELLIPSIS)
        text(hdc, row.detail, in: RECT(left: rect.left + px(14), top: middle + px(2), right: rect.right - px(14), bottom: rect.bottom - px(4)),
             font: smallFont, color: StripWindow.labelColor, format: DT_LEFT | DT_TOP | DT_SINGLELINE | DT_END_ELLIPSIS)
    }
    ...
    func handle(message: UINT, wParam: WPARAM, lParam: LPARAM) -> LRESULT? {
        ...
        case WM_CTLCOLOREDIT, WM_CTLCOLORLISTBOX:
            ...
            let color = isField ? StripWindow.addressFieldColor : StripWindow.backgroundColor
...
/// The same `GWLP_USERDATA` dance `stripWindowProc` does, for the panel.
private nonisolated func listPanelProc(
    ...
    let panel = Unmanaged<StripListPanel>.fromOpaque(stored).takeUnretainedValue()
