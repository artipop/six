...
/// `NiriStripView` with `layout.isOverview` on, drawn in GDI from the same numbers.
///
/// Nothing here is a page. The overview is a way of looking at the whole rail, and a strip of live
...
/// shows the picture its page left (`RailThumbnails`) — which is also what the Mac draws there
...
extension RailWindow {
    ...
    func overviewCard(atX x: Int, y: Int) -> RailModel.OverviewCard? {
