...
/// (`PageDialogs`); this is that sheet, as an owned popup over the row, with the site in its caption.
/// Not `MessageBoxW`, for two reasons. `prompt()` needs a field, and Windows has no input box to give
/// it. And a message box is a modal loop: while it is up nothing drains the main queue (`StripLoop`),
...
@MainActor
final class StripPageDialog {
    typealias Kind = StripWebView.PageDialog.Kind
    ...
    /// The window first, then the row back in front, then the answer — which may put up the next
    ...
    private func paint() {
        ...
        fill(hdc, client, StripWindow.backgroundColor)
        SetBkMode(hdc, TRANSPARENT)
        // `DT_NOPREFIX`: a page's `&` is an ampersand, not a mnemonic.
        text(hdc, message, in: messageRect, font: textFont, color: StripWindow.textColor,
             format: DT_LEFT | DT_WORDBREAK | DT_EDITCONTROL | DT_NOPREFIX)
        if isPrompt {
            roundedRect(hdc, fieldPill, fill: StripWindow.addressFieldColor, border: StripWindow.addressBorderColor)
        }
        if hasCancel {
            roundedRect(hdc, cancelButton, fill: StripWindow.chipColor, border: StripWindow.addressBorderColor)
            text(hdc, "Cancel", in: cancelButton, font: buttonFont, color: StripWindow.textColor,
                 format: DT_CENTER | DT_VCENTER | DT_SINGLELINE)
        }
        roundedRect(hdc, okButton, fill: accent, border: accent)
        text(hdc, "OK", in: okButton, font: buttonFont, color: StripWindow.rgb(255, 255, 255),
             format: DT_CENTER | DT_VCENTER | DT_SINGLELINE)
    }
    ...
    func handle(message: UINT, wParam: WPARAM, lParam: LPARAM) -> LRESULT? {
        ...
        case WM_PAINT:
            ...
            return 0
        case WM_LBUTTONUP:
            let x = SixStripPointX(lParam)
            let y = SixStripPointY(lParam)
        ...
        case WM_CTLCOLOREDIT:
            ...
            if fieldBrush == nil { fieldBrush = CreateSolidBrush(StripWindow.addressFieldColor) }
            SetTextColor(hdc, StripWindow.textColor)
            SetBkColor(hdc, StripWindow.addressFieldColor)
        ...
        case WM_DESTROY:
            ...
            if let hwnd { SixStripSetUserData(hwnd, nil) }
            hwnd = nil
...
private nonisolated func pageDialogProc(
    ...
    let dialog = Unmanaged<StripPageDialog>.fromOpaque(stored).takeUnretainedValue()
...
extension StripWindow {
    ...
    private static func name(of kind: StripWebView.PageDialog.Kind) -> String {
        switch kind {
        case .alert: "alert"
    ...
    /// `<input type=file>`: the system's own picker, owned by the row.
    ///
    ...
    func chooseFiles(_ choice: StripWebView.FileChoice, tabID: Foundation.UUID) {
    ...
    /// `GetOpenFileNameW`, through `SixStripOpenFiles`. `nil` is Cancel.
        ...
        let chosen = filter.withUnsafeBufferPointer { filterPointer in
            buffer.withUnsafeMutableBufferPointer { bufferPointer in
                title.withCString(encodedAs: UTF16.self) { caption in
                    SixStripOpenFiles(owner, caption, filterPointer.baseAddress, multiple ? 1 : 0,
