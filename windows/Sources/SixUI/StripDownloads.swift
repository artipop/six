...
/// C API has downloads: a navigation answered "download" (`StripWebView`'s navigation client) becomes
...
final class StripDownloads {
    ...
    func open(_ row: String) {
        ...
        _ = destination.withCString(encodedAs: UTF16.self) { SixStripShellOpen($0) }
    }
    ...
    ///
    /// Called from Swift rather than from `CStripInterop`: the declaration lives in `WinSDK.Shell`, and
    ...
    private static func address(of download: WKDownloadRef) -> String {
        ...
        return StripWebView.takeString(WKURLCopyString(url))
    }
...
/// `clientInfo`, alive in `StripDownloads.transfers` for exactly as long as the download is.
@MainActor
private final class Transfer {
    let id: Foundation.UUID
    let download: WKDownloadRef
    weak var store: StripDownloads?

    init(id: Foundation.UUID, download: WKDownloadRef, store: StripDownloads) {
        self.id = id
        self.download = download
        self.store = store
        // Held until the download says how it ended; `StripDownloads.end` lets it go.
    ...
    func install() {
        ...
        client.decideDestinationWithResponse = { _, _, suggested, clientInfo in
            ...
            MainActor.assumeIsolated {
                let name = StripWebView.string(from: offered)
        ...
        client.didFailWithError = { _, error, _, clientInfo in
            ...
            let transfer = Unmanaged<Transfer>.fromOpaque(clientInfo).takeUnretainedValue()
            MainActor.assumeIsolated {
                let message = failure.map { StripWebView.takeString(WKErrorCopyLocalizedDescription($0)) } ?? ""
        ...
        let panel = StripListPanel(
            ...
            rows: { [weak self] _ in
                self?.downloads.items.map {
                    StripListPanel.Row(id: $0.id.uuidString, title: $0.filename, detail: StripDownloads.detail(of: $0))
                } ?? []
            },
