...
///
/// A second `StripSandbox`, not a share of the translator's. The two hold different programs and
...
/// then.
@MainActor
final class StripEmbedding {
    private let sandbox = StripSandbox()

    /// Gives `StripModel` the two calls it needs and nothing else. The closures hold the sandbox, so
...
extension StripWindow {
    ...
    func bookmarkFocusedPage() {
        ...
        model.toggleFocusedPageBookmark(runScript: { [weak view] body, arguments in
            guard let view else { throw StripScriptError.noPage }
