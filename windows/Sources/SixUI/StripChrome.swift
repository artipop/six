...
/// both the painting and the hit-testing** — the same discipline `StripRendering.cardRect` keeps for
...
extension StripWindow {
    ...
    enum Metric {
        ...
        /// a workspace indicator, the Mac's `WorkspacePips`: one capsule per workspace, the
        ...
        /// A card's own title strip. `StripLiveView` insets the `WKView` below it, which is what
    ...
    /// What the row below the bar does not get. `TilingLayout`'s viewport is the row's canvas
    ...
    /// and goes. `StripLiveView.updateLiveView` notices when it changes and hands the layout its new
    ...
    func chromeLayout() -> ChromeLayout {
        ...
        // together — it is what the *row* has to be pushed down by — and using it here put every
        ...
        // title bar (`StripFrame`). Everything else in the cluster is measured from here.
    ...
    /// Where you are in the stack of workspaces: the workspace indicator, laid out horizontally, and
