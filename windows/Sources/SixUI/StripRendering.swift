...
/// GDI painting for the row, and the geometry `StripInput` and `StripLiveView` borrow back — so
/// painting, hit-testing and the live view can never drift apart on where a card actually is.
/// The bar above the row is `StripChrome`'s; the palette, the fonts and the metrics live there too.
extension StripWindow {
    ...
    /// rather than its position, so a column keeps its colour when the row reorders it.
    ...
    /// A column's frame in real window coordinates. `TilingLayout` lays columns out from `y = 0` like
    /// every other front, so the chrome above the row is added here, once, rather than baked into
    ...
    func bodyRect(for column: StripModel.Column) -> RECT {
    ...
    /// Painted into a memory bitmap and blitted once. The row redraws whole on every focus change,
    ...
    private func drawEverything(_ hdc: HDC, client: RECT) {
        ...
        if model.isOverview {
            ...
            drawEmptyStripHint(hdc, client: client)
        } else {
    ...
    /// What an empty workspace says, and the Mac's `EmptyWorkspaceHint` says it too: an empty row
    ...
    private func drawEmptyStripHint(_ hdc: HDC, client: RECT) {
        let width = px(200)
        let height = px(38)
    ...
    private func draw(_ column: StripModel.Column, hdc: HDC) {
        let card = cardRect(for: column.frame)
