...
extension RailWindow {
    ...
    func offerExternalLink(_ address: String, gesture: Bool, from tabID: Foundation.UUID) {
        ...
        case .none:
            ...
            askPage(RailWebView.PageDialog(host: host, message: "No app on this computer opens \(scheme): links.",
        ...
        askPage(RailWebView.PageDialog(host: host, message: "\(question)\n\n\(shown)", kind: .confirm) { value in
            guard value != nil else { return }
            _ = address.withCString(encodedAs: UTF16.self) { SixRailShellOpen($0) }
