...
final class StripTranslation {
    ...
    func consider(_ view: StripWebView, tabID: Foundation.UUID) {
        ...
        Task { @MainActor in
            ...
            // the moment a page has text in it — the same lateness `StripWindow` polls titles for —
    ...
    /// said, put it back.
    func toggle(_ view: StripWebView, tabID: Foundation.UUID) {
    ...
    private func start(_ view: StripWebView, tabID: Foundation.UUID, source known: Locale.Language?) async {
        let target = target
        var source = known
        if source == nil {
    ...
    /// Repaint four times a second while a run is going on. The same rate `StripWindow` polls the
...
extension StripWindow {
    /// Translate the page on screen, or put it back. Nothing happens in a row with no focused
