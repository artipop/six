...
///
/// the rule is that a named workspace survives running out of windows, and six kept it for as
...
/// stops being true.
@MainActor
struct TilingLayoutWorkspaceNameTests {
    ...
    private func named(_ layout: TilingLayout, _ name: String) -> (index: Int, window: UUID) {
    ...
    @Test func answeringYesTakesTheRow() {
        ...
        #expect(!names(layout).contains("research"))
        #expect(layout.workspaceToRemove == nil)
        // And dynamic workspaces still hold: the row with a window, and one empty one at the end.
    ...
    /// An unnamed row is not asked about, ever. It is what closing the last window in a row does a
    ...
    /// Several rows can empty at once — a profile being cleared, a row being taken apart — and each
