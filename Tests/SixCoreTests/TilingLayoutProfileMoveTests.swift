...
@MainActor
struct TilingLayoutProfileMoveTests {
    ...
    private func columns(_ layout: TilingLayout, in profile: UUID, workspace: Int = 0) -> [UUID] {
        layout.strip(for: profile).workspaces[workspace].columns.map(\.tabID)
    }

    /// The move as it happens on screen: the window leaves the row being looked at and is on the
    /// other profile's row when that one comes up.
    ...
    /// looking at. Left in, the column would be a place in a row pointing at a window that now
