...
| прогон | шагов | время | обращений к LLM | согласие с System 1 | итог |
|---|---|---|---|---|---|
| рейсы, только модель (адрес не задан) | 14 | 30.4 с | 14 | — | заявка верная |
| рейсы, гибрид, CPU, порог 0.9 | 12 | 57.7 с | 12 | 0 / 12 | верная |
| рейсы, гибрид, CPU, порог 0.5 | 16 | 41.0 с | 12 | 1 / 12 | верная |
| рейсы, гибрид, **MPS + исправленный запрос**, порог 0.9 | 14 | 44.8 с | 13 | 1 / 12 | верная |
| рейсы, то же, один проход вместо чанков | 15 | 39.0 с | 14 | 0 / 14 | верная |
| форма поддержки, гибрид, MPS | 9 | 24.7 с | 8 | 1 / 6 | payload совпал |

Три вещи по дороге выяснились и стоят отдельного упоминания, потому что две из них были ошибками этой стороны:
...
- **Запрос был обеднённым.** Варианты уходили как `{element, operation}`, тогда как jev-ultrafast шлёт
  ...
  роли и текущего значения в вопросе не было, модель отвечала одним и тем же `SELECT`. После починки она
  впервые стала брать шаги сама и верно: `Cabin → Business` при 0.90, поле «Where to?» при 0.91, «Full name»
  при 0.94.
- **Устройство.** Сервер грузил модель на CPU (0.3–2 с на шаг); `device="mps"` даёт 0.35–1.3 с. Заявленные
  автором 17–23 мс — это RTX 4070 Ti Super с TileLang, и разрыв с ноутбучным MPS в двадцать раз.
- **Ширина вопроса ни при чём.** Гипотеза, что порог не достигается из-за coarse-to-fine (вероятности
  перемножаются на двух проходах), проверена: один проход дал согласие 0 из 14 вместо 1 из 12.

**Вывод, который стоит записать честно: с laya-browser v10s гибрид пока дороже, чем чистая модель.** Согласие
с ней — 1 шаг из 12, то есть экономии токенов не возникает: модель всё равно зовут почти каждый шаг. При этом
измерять стало чем, и цена вопроса для следующего кандидата — адрес и ключ: клиент говорит на том же протоколе,
что и hosted Jev, чьи собственные замеры на Google Flights — 7 секунд на всю задачу против наших сорока.
Порог 0.9 по умолчанию и пустой адрес из коробки: цикл работает на одной модели, пока классификатор не
предъявит выигрыш на этих же задачах.

Что осталось непроверенным: **Jev вживую** (нужен ключ TypeSafe) и второй файнтюн,
