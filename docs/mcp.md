...
| `list_console_messages` | what a window's page logged since it last navigated (`level`, `limit`); listed only while `six://configuration` ▸ Assistant ▸ Let Agents Read Page Console and Network is on — see [devtools.md](devtools.md) |
...
| `call_page_tool` | calls one of them (`name`, `arguments` as an object or its JSON text, `timeout` seconds — default 30, `max_chars`); the answer comes back fenced as the page's data. Ends with the page's answer, a timeout (the tool's `AbortSignal` fires in the page), or the page navigating away |
