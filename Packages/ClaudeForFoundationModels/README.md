...
**Vendored copy (six):** tag 0.1.4 of https://github.com/anthropics/ClaudeForFoundationModels with one patch —
`GenerationOptions.SamplingMode.Kind` cases renamed to `.top(k:seed:)` / `.nucleus(threshold:seed:)` to match the
macOS 27 SDK shipped with Xcode 27A5209h. Replace with the upstream remote package once a release matches the installed SDK.
