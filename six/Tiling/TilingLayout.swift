...
///
/// A column is one screen's worth of row whether it holds one window or two: a split shares the
/// width one window would have had, with a gap down the middle half the size of the one between
/// columns, so the pair reads as belonging to each other rather than as two neighbours in the row.
///
/// A tiling window manager splits a column the other way — its windows stack vertically — and that
/// is right for terminals and wrong for pages. A web page is tall; two half-height ones are two pages nobody can
...
/// pages, and wanting more than two things at once is the question the row already answers.
nonisolated struct TilingColumn: Identifiable, Hashable, Sendable, Codable {
    ...
    /// The side keeps the order the row had: a window pulled in from the left arrives on the left.
...
nonisolated struct TilingWorkspace: Identifiable, Sendable, Codable {
    var id = UUID()
    /// Optional. A named workspace does not vanish the moment it runs
    /// out of windows: it is asked about first (`TilingWorkspaceRemoval`), because the name is the one
    /// thing in a row a person typed and losing it silently is losing work.
    var name: String = ""
...
/// a screen's worth of page in both: the row leaves it the gaps it needs to read as a card in a row
...
/// what the row's `WebView` does.
nonisolated enum TilingFill: String, Sendable, Codable {
    /// The row as usual: the gaps, and a window as wide as the screen leaves room for.
    case tiled
...
/// Which side of the focused column a new window opens on. Right stays the
...
///
/// A row with nothing in it is not worth keeping — that is the rule and six's, and it is why an
...
/// in a row a person put there in words, and taking it away without saying so is taking away work.
///
/// So the rule is the same everywhere and the question is the whole of the difference: the row is
/// gone if the answer is yes, and stands if it is no. `TilingLayout` only asks; how the question looks
...
nonisolated enum TilingColumnSide: Equatable, Sendable {
    case whole
    case left
    case right
}
...
nonisolated struct TilingWindowPlace: Identifiable, Sendable, Equatable {
    var tabID: UUID
    var frame: CGRect
    var side: TilingColumnSide
    var id: UUID { tabID }
}

/// A side of the canvas, as the row feels it when there is nothing behind it. Named for the
/// gesture rather than for the screen: `leading` is the end of the row you scroll back towards,
/// `above` the workspace you scroll up to.
nonisolated enum TilingEdge: Sendable, Hashable, CaseIterable {
    case leading
    case trailing
    case above
    case below
...
/// See `TilingLayout.dropTarget`.
nonisolated struct TilingDrop: Equatable, Sendable {
    var workspace: Int
    var index: Int
    var joins: UUID?
}

/// A column being carried across the overview: where it was picked up, how far the pointer has
/// travelled, and where it would land if it were let go now.
///
/// **What is in the hand is the column, not the window**, so a split travels as the pair it is:
/// either half picks both up, they land side by side wherever the drop lands, and the arrangement
/// survives the trip. Taking a pair apart is ⌥S, and a drag that quietly did it as well would undo
/// an arrangement it was never asked to touch — with no way back to it inside the same movement.
///
/// The strip itself is not touched until the drop. Until then this is what the canvas draws instead:
/// the carried column taken out of the row it came from, a gap of its size held open where it would
/// land, and the card itself following the pointer above both.
nonisolated struct TilingColumnDrag: Sendable, Equatable {
    /// The column in the hand, with both its windows when it has two — and with its own identity,
    /// which it keeps all the way to the drop, so the view that was drawing it goes on drawing it
    /// (`TilingColumn.id`).
    var column: TilingColumn
    /// The half the pointer took hold of: the window the focus lands on, and the one that joins
    /// another column when a single window is let go over one.
    var tabID: UUID
    ...
    /// makes a column of its own, and the only answer a carried *pair* can have: two is the ceiling.
    /// By id and not by index because the row the drop lands in is the row without the carried
    /// column in it, and every index past where it stood has moved.
    var joins: UUID?
    /// Which side of the window it would join it lands on: the side it is being held over.
    var joinsSide: TilingPlacement = .right
    ...
    /// Everything in the hand, left to right.
    var tabIDs: [UUID] { column.tabIDs }

    var movedSomewhere: Bool {
        if let joins { return joins != column.id }
...
/// Scrollable tiling: columns laid out left to right in a workspace, workspaces stacked
...
final class TilingLayout {
    ...
    /// moves — the lean, the glyph, the promise at the end of the row — is this one spring, so the
    /// whole curtain is a single motion.
    static let peekAnimation: Animation = .smooth(duration: 0.4)
    /// How far a gesture has to push at an end of the row for the wall to be fully lit — in points,
    /// like the threshold that produces it, because this is a measure of a finger and not of a
    /// screen: it is `TilingScrollMonitor.threshold` (55) through the rubber band's 0.35, the whole
    ...
    /// What is left of the rubber band at an end of the row. A band that gives as freely where there
    ...
    /// The row goes on working underneath, so ⌥←/⌥→ walks from one full-width window to the next.
    /// Not macOS fullscreen (the green button), and not a page asking for `requestFullscreen`, which
    /// WebKit handles on its own inside the web view. This is only the value before anything has been
    /// read into it; `BrowserState.init` overwrites it with what `ConfigurationStore.fill` remembers.
    private(set) var fill: TilingFill = .tiled
    /// Park the focused window in the middle of the screen instead of
    /// scrolling as little as possible. Off means the row only moves when the focus would fall off it.
    ...
    var edgeHover = 0
    /// A window being carried across the overview, or `nil`. See `TilingColumnDrag`.
    var columnDrag: TilingColumnDrag?
    ...
    /// The side the row was last pushed into with nothing behind it, and how brightly it is lit
    /// (0…1). See `hitWall`.
    private(set) var wall: TilingEdge?
    private(set) var wallGlow: CGFloat = 0
    /// The windows ⌥⇧ just moved, held up out of the row until the move has landed — two of them when
    /// what moved was a split, since the pair travels as one thing. See `lift`.
    private(set) var liftedTabIDs: Set<UUID> = []
    ...
    /// leaving the column, the way a tiling WM walks the windows of a column — but in this row both
    /// halves are on screen side by side, so that step moved nothing and the key read as dead. It
    /// also left the end of a row whose last column was a split with a chevron on both sides: the
    /// strip leaned over empty canvas, and the `+` that grows the row had nowhere to appear. The
    ...
    /// The overview is another way of looking at the same row, so filling steps aside while it is
    /// open — with the gaps and the title bars back, the columns can be told apart — and comes back
    /// when it closes.
    var showsFill: TilingFill { isOverview ? .tiled : fill }
    ...
    /// One window, one screen. Every column in the row is this wide — the viewport with the outer
    ...
    func columnFrames(_ workspace: TilingWorkspace) -> [CGRect] {
    ...
    func columnFrames(_ columns: [TilingColumn]) -> [CGRect] {
        var frames: [CGRect] = []
        var x = outerGap
        for _ in columns {
    ...
    /// between columns: at the row's own gap a split would be indistinguishable from two windows
    /// standing next to each other. Half of it, so proximity says what no chrome has to — these two
    /// are closer to each other than to anything else in the row — with a floor, because at full
    /// width the row's gap is zero and two pages flush against each other have no seam at all.
    ...
    /// halves worked out. What the strip draws from; see `TilingWindowPlace`.
    func placements(_ columns: [TilingColumn]) -> [TilingWindowPlace] {
        let frames = columnFrames(columns)
        var places: [TilingWindowPlace] = []
        for (index, column) in columns.enumerated() where frames.indices.contains(index) {
            ...
            for (pane, tabID) in column.tabIDs.enumerated() where panes.indices.contains(pane) {
                places.append(TilingWindowPlace(
    ...
    func paneFrames(_ column: TilingColumn, in frame: CGRect) -> [CGRect] {
    ...
    func contentWidth(_ workspace: TilingWorkspace) -> CGFloat {
    ...
    /// overview's `+`. Not `newColumnFrame(at:)`, which is about the focused row and the end the
    ...
    /// An empty row has no row to stand at the end of: its content is a point at the middle of the
    /// screen, and a place hung half to each side of that point is a place in the middle.
    private func appendX(_ workspace: TilingWorkspace) -> CGFloat {
    ...
    /// shows past the right edge — the same half-a-window-over-there the row leans to reveal.
    func overviewWidth(_ workspace: TilingWorkspace) -> CGFloat {
    ...
    /// row as it *stands*, not as it is drawn mid-drag: a press happens before there is a drag.
    func canvasPlaces() -> [TilingWindowPlace] {
        var places: [TilingWindowPlace] = []
    ...
    private func offsetBounds(in workspace: TilingWorkspace) -> ClosedRange<CGFloat> {
        ...
        // A row too long to fit scrolls, and the `+` past its last window has to be reachable.
        return 0...(total - width + (isOverview ? columnWidth / 2 : 0))
    }

    private func clampOffset(_ offset: CGFloat, in workspace: TilingWorkspace) -> CGFloat {
    ...
    func resolvedOffset(_ workspace: TilingWorkspace) -> CGFloat {
    ...
    private func scrollFocusIntoView(_ workspace: inout TilingWorkspace) {
        let frames = columnFrames(workspace)
    ...
    func setFill(_ value: TilingFill) {
    ...
    /// Opens a tab as a new column to the right of the focused one.
    ...
    func peek(_ amount: CGFloat? = nil) {
        ...
        peekTask = Task { @MainActor [weak self] in
            ...
            TilingLayout.trace("peek out \(horizontalPreview)")
            try? await Task.sleep(for: .milliseconds(260))
            guard !Task.isCancelled else { return } // a newer peek owns the band now, and will let go
            withAnimation(TilingLayout.switchAnimation) { self.horizontalPreview = 0 }
            TilingLayout.trace("peek back \(horizontalPreview)")
        }
    }

    /// ⌥⇧ moves the window you are reading, and in the row that is the one move you cannot see: the
    /// row follows the window, so it stands exactly where it stood and only what is around it changes
    /// — off screen, at full width. The overview shows it, because there the window is a card among
    /// cards. This borrows that for the length of the move: the window comes up out of the row a little,
    /// with corners, so the row can be seen going past behind it, and settles back once it has landed.
    private func lift(_ tabIDs: [UUID], animated: Bool, holding hold: Int = 340) {
        guard !isOverview else { return }
        liftTask?.cancel()
        let lifted = Set(tabIDs)
        if animated {
            withAnimation(.smooth(duration: 0.2)) { liftedTabIDs = lifted }
        } else {
            liftedTabIDs = lifted
        }
        liftTask = Task { @MainActor [weak self] in
            // The length of `switchAnimation`: set down once the row has arrived, not while it moves.
            try? await Task.sleep(for: .milliseconds(hold))
            guard !Task.isCancelled, let self else { return }
            withAnimation(.smooth(duration: 0.35)) { self.liftedTabIDs = [] }
        }
    }

    // MARK: The end of the row

    /// The row was asked to go where there is nothing, and the light along that edge is the whole
    /// answer.
    ///
    /// A row that simply refuses looks like a row that did not hear: the strip does not move, the
    ...
    ///
    /// Deliberately not a sound and not a bounce. A bounce is the row moving, and the one thing that
    /// must stay true is that it did not.
    func hitWall(_ edge: TilingEdge) {
        wallTask?.cancel()
        wall = edge
        withAnimation(.smooth(duration: 0.12)) { wallGlow = 1 }
        TilingLayout.trace("wall \(edge)")
    ...
    /// with it, so pushing gently at the end of the row says the same thing quietly. `nil` is the
    ...
    /// the row refused, divided by `wallPush`.
    func pushWall(_ edge: TilingEdge?, by progress: CGFloat) {
        wallTask?.cancel()
        wallTask = nil
        guard let edge else {
            guard wallGlow != 0 else { return }
            withAnimation(TilingLayout.switchAnimation) { wallGlow = 0 }
            return
        }
        wall = edge
        ...
        // light stayed in a row that had not reached its end. A push is a thing a hand is doing, so
        ...
        wallTask = Task { @MainActor [weak self] in
            ...
            withAnimation(TilingLayout.switchAnimation) { self.wallGlow = 0 }
        }
    }
    ...
    private func wallEdge(column delta: Int) -> TilingEdge? {
        guard !canFocusColumn(delta) else { return nil }
        return delta > 0 ? .trailing : .leading
    }

    private func wallEdge(workspace delta: Int) -> TilingEdge? {
    ...
    /// of the row. The view hands over the raw band and gets both: a band that gives less where
    ...
    /// of lean and then the row jumped a column, and the row read as heavy: you push, almost
    /// nothing happens, and then it teleports. A window is a screen wide, so a gesture that is
    /// half-way to the next one should have moved the row half a screen. The commit is then seamless
    ...
    /// negative band is the row leaning towards its far end.
    func previewColumn(_ progress: CGFloat) {
        guard progress != 0 else {
            pushWall(nil, by: 0)
            withAnimation(TilingLayout.switchAnimation) { horizontalPreview = 0 }
            return
        }
    ...
    func previewWorkspace(_ progress: CGFloat) {
        guard progress != 0 else {
            pushWall(nil, by: 0)
            withAnimation(TilingLayout.switchAnimation) { verticalPreview = 0 }
            return
        }
    ...
    /// Where a window opened at one end of the row would stand, in content space — **whether or not
    /// anybody is looking at that end**. The place is a property of the row, not of the pointer:
    ...
    /// spring before the row got there — which is the second peek people were seeing.
    ...
    func focusColumn(_ delta: Int) {
        ...
        mutate { s in
            ...
            // something the hand said by clicking, and nothing the row should quietly change.
            ws.focus = target
            scrollFocusIntoView(&ws)
            s.workspaces[s.focus] = ws
        }
    ...
    func moveColumn(_ delta: Int) {
        var moved: [UUID] = []
        defer { if !moved.isEmpty { lift(moved, animated: true) } }
        mutate { s in
            guard s.workspaces.indices.contains(s.focus) else { return }
            var ws = s.workspaces[s.focus]
            ...
            ws.focus = target
            // The whole column changes places, so the whole column comes up out of the row.
            moved = ws.columns[target].tabIDs
            scrollFocusIntoView(&ws)
            s.workspaces[s.focus] = ws
        }
    }
    ...
    /// being two windows in the row.
    ///
    /// Which window it takes is the one the row would have walked to — the neighbour on the right,
    /// or the one on the left when the focused window is the last in the row, so the end of a row
    ...
    /// it. Nothing is closed either way — a split is an arrangement of windows already in the row.
    ///
    ...
    func toggleSplit() -> Bool {
        ...
        // torn down any more — the strip draws by window (`TilingWindowPlace`), so a split is one view
        ...
        unanimated {
            mutate { s in
                ...
                if ws.columns[ws.focus].isSplit {
                    ...
                    column.remove(right)
                    ws.columns[ws.focus] = column
                    ws.columns.insert(TilingColumn(tabID: right), at: ws.focus + 1)
                    if followed == right { ws.focus += 1 }
                } else {
                    let side: TilingPlacement = ws.columns.indices.contains(ws.focus + 1) ? .right : .left
    ...
    ///
    /// The first window does not move: it keeps its column and its place in the row, and the second
    ...
    ///
    /// For anything that draws a window **away from the row** and has to draw it as the thing it
    /// actually is: the ⌃Tab ring above all, where a card is a picture of a column and a split is two
    /// pictures in one card, the way it looks in the row.
    ...
    func arrangement(workspaceAt index: Int) -> [TilingColumn] {
        ...
        // sentence said inside a column. A pair in the hand has no such answer — two is the ceiling —
        // so it always stands as a column of its own.
        if let joins, !drag.column.isSplit,
           let at = columns.firstIndex(where: { $0.id == joins }), !columns[at].isSplit {
            columns[at].insert(drag.tabID, on: drag.joinsSide)
        } else {
            columns.insert(drag.column, at: min(max(0, drag.toIndex), columns.count))
        }
        return columns
    }

    /// The row a drop would land in: the workspace's own columns, less the one in the air — all of
    /// it, both halves included, because that is what was picked up.
    private func rowUnderDrag(_ drag: TilingColumnDrag, workspace index: Int) -> [TilingColumn] {
        var columns = workspaces[index].columns
        guard index == drag.fromWorkspace, let at = columns.firstIndex(where: { $0.id == drag.column.id })
        else { return columns }
    ...
    /// The frame the picked-up card had before it moved, in its own row's content space — a whole
    /// column's worth of it, since a column is what is being carried, split or not.
    private func liftedFrame(_ drag: TilingColumnDrag) -> CGRect? {
    ...
    /// Picks up the column this window is in — the pair, when it is sharing one. Either half takes
    /// hold of the whole card; which of the two was grabbed is remembered, because it is the window
    /// the focus lands on.
    func beginColumnDrag(tabID: UUID) {
        guard isOverview, let at = location(of: tabID) else { return }
        var column = workspaces[at.workspace].columns[at.index]
        column.pane = column.paneIndex(of: tabID) ?? 0
        columnDrag = TilingColumnDrag(
            column: column, tabID: tabID, fromWorkspace: at.workspace, fromIndex: at.index,
    ...
    func updateColumnDrag(translation: CGSize) {
        ...
        var index = 0
        var joins: UUID?
        var side: TilingPlacement = .right
        for (position, other) in frames.enumerated() where row.indices.contains(position) {
            let column = row[position]
            // The column in the hand is not in the row the drop lands in, so it is neither a place
            // to count past nor one to join.
            if drag.toWorkspace == drag.fromWorkspace, column.id == drag.column.id { continue }
            ...
            // A pair in the hand has nowhere to be put *inside* another column, so the middle of a
            // window means nothing to it: two is the ceiling, and it would be a third and a fourth.
            guard !drag.column.isSplit, !column.isSplit else { continue }
    ...
    var dropSlotFrame: CGRect? {
        ...
        let places = placements(arrangement(workspaceAt: drag.toWorkspace))
            .filter { drag.tabIDs.contains($0.tabID) }
        guard let first = places.first else { return nil }
        // Both halves of a carried pair, as one rectangle: what is being put down is the column, and
        // an outline around each half would draw it as two windows that happen to be adjacent.
        let frame = places.dropFirst().reduce(first.frame) { $0.union($1.frame) }
        return CGRect(x: canvasX(content: frame.minX, workspace: drag.toWorkspace),
                      y: rowY(drag.toWorkspace) + frame.minY,
    ...
    var dropTarget: TilingDrop? {
        columnDrag.map { TilingDrop(workspace: $0.toWorkspace, index: $0.toIndex, joins: $0.joins) }
    }
    ...
    /// Drops the carried column where the drag left it, and returns whether the strip changed.
    @discardableResult
    func commitColumnDrag() -> Bool {
        ...
        mutate { s in
            guard s.workspaces.indices.contains(drag.fromWorkspace),
                  let at = s.workspaces[drag.fromWorkspace].columns.firstIndex(where: { $0.id == drag.column.id })
            else { return }
            // The column as the strip has it, not as the drag remembers it: a window closed while
            // the card was in the air is a half the pair no longer has.
            var column = s.workspaces[drag.fromWorkspace].columns[at]
            column.pane = column.paneIndex(of: drag.tabID) ?? column.pane
            ...
            if let joins = drag.joins, !column.isSplit,
               let onto = destination.columns.firstIndex(where: { $0.id == joins }),
    ...
    func panStrip(by delta: CGFloat) {
        ...
        mutate { s in
            ...
            // that, since it shows far more of the row than the window does — and adding to the
            ...
            // How much of the push the row could not take. A free pan has no rubber band to give —
    ...
    func snapFocusToView() {
        pushWall(nil, by: 0) // the fingers are out of the row, so the wall stops being pushed
        mutate { s in
            guard s.workspaces.indices.contains(s.focus) else { return }
            var ws = s.workspaces[s.focus]
    ...
    /// Moves the focused column — the pair, when it is a split — to the workspace above/below and
    /// follows it.
    ///
    ...
    /// window was lifted and set down again in a row that had not visibly moved. Returns the row the
    /// window landed in.
    @discardableResult
    func carryColumn(toWorkspace delta: Int) -> UUID? {
        ...
        unanimated {
            mutate { s in
                ...
                // What moves is the column in front of you, both halves of it. Two pages put side by
                // side are one thing a person arranged, and a key that carried one of them off to
                // another row would take that arrangement apart on the way — with the pair now a
                // workspace apart, and no way back to it but making it again. ⌥S is how they come
                // apart, and it leaves them both where they can be seen.
                let column = source.columns[source.focus]
                let moved = column.tabIDs
                ...
                if target >= s.workspaces.count { s.workspaces.append(TilingWorkspace()) }
                var destination = s.workspaces[target]
                let index = destination.columns.isEmpty ? 0 : destination.focus + 1
                destination.columns.insert(column, at: min(index, destination.columns.count))
