...
/// The tiling canvas: every workspace is a full-screen horizontal strip of columns; the workspaces are
/// stacked vertically and only one of them is on screen at a time.
struct TilingStripView: View {
    ...
    @State private var monitor = TilingScrollMonitor()
    ...
    private func carries(_ workspace: TilingWorkspace, layout: TilingLayout) -> Bool {
    ...
    private func offset(of index: Int, height: CGFloat, layout: TilingLayout) -> CGFloat {
    ...
    private func startMonitor() {
        ...
        // when there is nothing behind the edge being pushed (`TilingLayout.previewColumn`), is the same
...
private struct WorkspaceView: View {
    ...
    var body: some View {
        ...
        // `TilingLayout.unanimated` was written for, arriving from the other side — there a window
    ...
    @ViewBuilder
    private func canvas(layout: TilingLayout, row: Int) -> some View {
        ...
        // row it would land in (`TilingLayout.arrangement`). The card itself is drawn above the canvas,
        ...
        ZStack(alignment: .topLeading) {
            ...
            // The window a `+` would open, standing at both ends of the row where it would open —
            ...
            // it (`TilingLayout.newColumnFrame(at:)`).
            if isCurrent {
                ForEach([-1, 1], id: \.self) { side in
                    if let place = layout.newColumnFrame(at: side) {
                        ...
                        NewColumnGhost(filled: layout.fillsViewport,
                            ...
                            .opacity(shown ? 1 : 0)
                            .animation(TilingLayout.peekAnimation, value: shown)
            ...
            // one — the trap `TilingWindowPlace` is written up in, and the one this crashed on. Here a
            ...
            ForEach(places) { place in
                if let tab = browser.tab(place.tabID), !carried.contains(place.tabID) {
                    ...
                    // the width: the *offset* is the row scrolling, which is the movement the
                    // animation is for.
        ...
        // Clipped to the row in the overview, where rows sit side by side on one screen. In the row
    ...
    private func isLive(workspaceDistance: Int, x: CGFloat, width: CGFloat, layout: TilingLayout) -> Bool {
...
/// What an empty workspace shows in the row: a button for its first window.
///
/// Only in the row. In the overview it was drawn with the button switched off, because `newTab`
...
/// making a window up there, since ⌘T and the row's `+` both open beside a focus that may be three
...
///
/// What it promises is drawn the way the row's own `+` promises it — the start page in miniature
...
private struct NewWindowPlace: View {
    let workspace: Int
    /// An empty row's place stands in the middle of the screen rather than past the end of a row
...
/// (`TilingLayout.arrangement`).
///
/// **A split is carried as the pair it is**, so the card is the two windows side by side, at the
/// spacing they have in the row — drawn with one shadow under both, because what is in the hand is
...
private struct OverviewPointerLayer: View {
    ...
    var body: some View {
        ...
        // in (`TilingLayout.beginColumnDrag`).
    ...
    private func bounds(_ layout: TilingLayout) -> CGRect {
    ...
    private func card(_ cards: [TilingWindowPlace], at point: CGPoint) -> UUID? {
        cards.first { $0.frame.contains(point) }?.tabID
    }

    private func gesture(cards: [TilingWindowPlace]) -> some Gesture {
        DragGesture(minimumDistance: 0, coordinateSpace: .named(TilingStripView.canvasSpace))
            .onChanged { value in
                if carrying == nil {
...
private struct OverviewCloseButtons: View {
    ...
    var body: some View {
        ...
        // A share of the card as it is drawn, so a row of twenty does not get twenty full-size
        // buttons on it.
...
/// seam at a strength that changes every frame, and the seam shimmers for as long as the row moves.
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if tab.showsStartPage {
                ...
                WebView(page)
                    ...
                    // it when the row's focus arrives here (`WebViewResponder`). In the background,
        ...
        .overlay {
            if !filled {
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    ...
                    // changes — including the window's width, which nothing else in the row ever
        ...
        .overlay(alignment: .topTrailing) {
            if !filled, isCurrentWorkspace, !browser.layout.isOverview {
                ...
                // a row-gap away — so its × comes back inside its own corner rather than sitting on
...
/// card with no screenshot lived without: a globe and a line of text tell a row of windows apart
...
private struct ColumnPlaceholder: View {
    ...
    var body: some View {
        ...
        // many windows are in the row. An icon and a title that shrank with it would be a
        // different size in every row; divided by the scale they are the same size to the eye.
    ...
    var body: some View {
        ...
        if !layout.isOverview {
            GeometryReader { proxy in
                ...
                let _ = TilingLayout.trace("edge geom proxyW=\(proxy.size.width) viewportW=\(layout.viewport.width) frameMaxX=\(frame?.maxX ?? -1) left=\(left) right=\(right) edgeHover=\(layout.edgeHover) edgeLean=\(layout.edgeLean)")
            ...
            .animation(TilingLayout.switchAnimation, value: layout.focusedColumnFrame)
        }
    }
}
...
/// far enough to show what is over there (`TilingLayout.edgeHover`): the next window, or an outline of
...
private struct StripEdgeButton: View {
    ...
    static func lane(_ layout: TilingLayout) -> CGFloat {
    ...
    private func targetHeight(_ layout: TilingLayout) -> CGFloat {
    ...
    /// exactly as far as it leaned (`TilingLayout.edgeLean`), so the curtain the pointer sees is the
    ...
    /// per side.
    private func reveal(_ layout: TilingLayout) -> CGFloat {
    ...
    var body: some View {
        ...
        // `TilingStripView.swift:120` — stays a `peekAnimation` spring; only this hit box's own geometry
    ...
    @ViewBuilder
    private func content(layout: TilingLayout, width: CGFloat) -> some View {
        if let step = step(layout: layout) {
            Button {
                ...
                if step.opens {
                    ...
                    // The chevron that has just walked the row to its end: the curtain stays open
            ...
            .frame(width: width, height: targetHeight(layout))
            .onHover { inside in
                TilingLayout.trace("edge onHover dir=\(direction) inside=\(inside) width=\(width) anchorX=\(anchorX)")
                hovering = inside
            ...
            // quick.
            .animation(browser.peeksAtEdges ? TilingLayout.peekAnimation : .easeOut(duration: 0.15), value: hovering)
        }
    }

    private func step(layout: TilingLayout) -> (symbol: String, help: String, opens: Bool, action: () -> Void)? {
    ...
    private func peek(_ layout: TilingLayout, _ hovering: Bool) {
        withAnimation(TilingLayout.peekAnimation) { layout.hoverStripEdge(direction, hovering) }
    }
    ...
    @ViewBuilder
    private func glyph(_ symbol: String, layout: TilingLayout) -> some View {
...
/// The light along an edge the row was pushed into with nothing behind it (`TilingLayout.hitWall`).
///
/// It is the answer to a gesture that changed nothing, so it has to be visible without being an
/// event: a row that flashed a bar of colour every time you reached its end would be a row that
...
private struct StripWalls: View {
    ...
    var body: some View {
        ...
        // for something else to redraw it, which in a row that is a wall on both sides meant the
        ...
        if let lit, glow > 0.005 {
            GeometryReader { proxy in
                ...
                // edge, which is what it did: a row with one window (or none) is a wall on both
                ...
                ZStack {
                    ForEach(TilingEdge.allCases, id: \.self) { edge in
    ...
    private func band(_ edge: TilingEdge, glow: CGFloat, in size: CGSize) -> some View {
    ...
    private func alignment(_ edge: TilingEdge) -> Alignment {
        switch edge {
    ...
    private func from(_ edge: TilingEdge) -> UnitPoint {
        switch edge {
        case .leading: .leading
        case .trailing: .trailing
        case .above: .top
    ...
    private func to(_ edge: TilingEdge) -> UnitPoint {
        switch edge {
...
private struct WorkspacePlates: View {
    ...
    @ViewBuilder
    private func plate(index: Int, workspace: TilingWorkspace) -> some View {
        let layout = browser.layout
            ...
            if editing == workspace.id {
                ...
                Text(layout.title(at: index))
                    ...
                    .contextMenu {
                        ...
                        // asked — an old row, or an answer of "keep" that has since gone stale.
    ...
    private func startEditing(_ workspace: TilingWorkspace) {
